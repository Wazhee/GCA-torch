...
def random_analyze_aim_2(model, test_data, target_sex=None, target_age=None, augmentation=False, rate=None):
#   results, rate = [], -1
    results = []
    target_path = f'random_target_sex={target_sex}_age={target_age}'
    for trial in range(num_trials):
        y_true = pd.read_csv(f'splits/{test_data}_test.csv')
        if rate == 0:
            y_pred = pd.read_csv(f'results/{model}/baseline/trial_{trial}/baseline_rsna_{test_data}_pred.csv')
    ...
    #             threshold = __threshold(pd.read_csv(f'splits/rsna_test.csv')['Pneumonia_RSNA'].values, pd.read_csv(f'{get_weights_folder(path, trial)}')['Pneumonia_pred'].values)
            if augmentation:
                y_pred = pd.read_csv(f'results/{model}/augmented={augmentation}_{target_path}/trial_{trial}/poisoned_rsna_rate={rate[0]}&{rate[1]}_{test_data}_pred.csv')

                threshold = __threshold(pd.read_csv(f'splits/rsna_test.csv')['Pneumonia_RSNA'].values, pd.read_csv(f'results/{model}/augmented={augmentation}_{target_path}/trial_{trial}/poisoned_rsna_rate={rate[0]}&{rate[1]}_{test_data}_pred.csv')['Pneumonia_pred'].values)
        
            else:
                y_pred = pd.read_csv(f'results/{model}/{target_path}/trial_{trial}/poisoned_rsna_rate={rate[0]}&{rate[1]}_{test_data}_pred.csv')

                threshold = __threshold(pd.read_csv(f'splits/rsna_test.csv')['Pneumonia_RSNA'].values, pd.read_csv(f'results/{model}/{target_path}/trial_{trial}/poisoned_rsna_rate={rate[0]}&{rate[1]}_{test_data}_pred.csv')['Pneumonia_pred'].values)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-json', help='path to json file', type=str, default="random_F&0-20_0.15&0.73") 
    ...
    json_path = args.json
    with open('random_M&40-60_0.52&0.92.json') as json_file:
