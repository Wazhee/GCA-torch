...
class GCA():
    ...
    def __get_hyperplanes__(self):
        ...
        self.sex_coeff = self.__learn_linearSVM__(image_data["m"], image_data["f"], patient_data["m"], patient_data["f"])
        self.age_coeff = self.__learn_linearSVM__(image_data["y"], image_data["o"], patient_data["y"], patient_data["o"], key="Age")
        print("Sex and Age coefficient loaded!")
    
    def __sex__(w, step_size = 1, is_male=True):
        if is_male:
    ...
    def __sex__(self, w, step_size = 1, magnitude=2):
        ...
        new_w + alpha * self.sex_coeff
        synth, _ = self.generator([new_w], input_is_latent=True)
        # save images
        utils.save_image(synth, "real_samples.png", nrow=int(1 ** 2), normalize=True)
        return w + alpha * self.sex_coeff
        
    
    def augment(self, embedding, p=0.8): # p = augmentation rate
        if np.random.choice([True, False], p=[p, 1-p]): # random 80% chance of augmentation
            self._sex__(embedding, magnitude=random.randint(0,4))
